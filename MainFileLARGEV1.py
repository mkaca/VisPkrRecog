...
while(1):
        ...
        save_path = "C:\\Users\\dabes\\Desktop\\Vision Recognition Card Poker\\MySnapshot3.jpg"
        snapshot.save(save_path)
        image = cv2.imread("MySnapshot3.jpg")
        ...
        for c in cnts:
            ...
            if ((numpy.absolute(box[0,0]-box[3,0]) <40) and (numpy.absolute(box[1,1]-box[2,1]) <40)):
                    ...
                    if (numpy.absolute(box[0,1] - box[3,1]<3) or numpy.absolute(box[1,1] - box[2,1])<3):
                            ...
                            if (box[2,0] - box[1,0] <7 or box[3,0] - box[0,0] > 25 or box[1,1] - box[0,1] > 80):
                                continue
                            #print("box after: ", box)
                            #print(box)
                            """if (left):
                            ...
                            if (valid):
                                ...
                                if (area >300):
                                        if(box[1,0] > 575 and box[1,0] < 595 and box[1,1] - box[0,1]>18):
                                                cv2.drawContours(croppedImg,[box],0,(0,0,255),1)
                                                print (box)
                                                # add to array
                                                #cntsCardsFound.append(c)
                                                cntsCardsFound.append(box)
                                                if xRef == 0 or xRef> box[0,1]:  ### this gets the bottom left card as the ref point
                                                        xRef = (box[0,1])
                                                        yRef = (box[0,0])
                                        
        if len(cntsCardsFound) >0:
                print (cntsCardsFound[0])
        ...
        # MIGHT HAVE TO MOVE AROUND A LITTLE BIT TO GET THE CARD # TO BE IN THE MIDDLE
        croppedGrayImg = cv2.cvtColor(croppedImg, cv2.COLOR_BGR2GRAY)
        centralCards = []
        bitCentralCards = []
        myCards = []
        bitMyCards = []
        ...
        x = xRef+15 #x = 585
        y = yRef-16 #y = 584
        #gets location of my cards
        for i in range(2):
                cv2.rectangle(croppedImg, (x, y), (x+width, y+ height), (255, 255, 0), 1)
                myCards.append(croppedGrayImg[y:y+height,x:x+width])
                x = x+82
                th3 = cv2.adaptiveThreshold(myCards[i],255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,\
                cv2.THRESH_BINARY,11,2)
                bitMyCards.append(th3)
                print (bitMyCards[i])
                print ('')
                
        y = yRef - 265-16 #y = 319
        x = xRef - 141+15 #x = 444
        # Gets location of middle cards
        for i in range(5):       
                cv2.rectangle(croppedImg, (x, y), (x+width, y+ height), (255, 255, 0), 1)
                centralCards.append(croppedGrayImg[y:y+height,x:x+width])
                x = x+91
                th3 = cv2.adaptiveThreshold(centralCards[i],255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY,11,2)
                bitCentralCards.append(th3)
                print (bitCentralCards[i])
                print ('')
        #######################################################################################################################
	...
	[0, 80, 0], [20, 255, 150]]   ### THIS ONE WORKS NICELY FOR GREEN.... LOWER, UPPER respectively
        ...
        ############# HARDCODE THE CARD LOCATION BASED ON RELATIVE POSITION OF THE CARD/ CARDS that you find with contours
        # HENCE IT TELLS YOU IF IT IS YOUR TURN
        #THEN you look at your cards, and compare specific points in the array 
"""
